  ...
  const filteredAppointments = appointments
    .filter(appt => {
      const matchesSearch = appt.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        (appt.organization && appt.organization.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (appt.venue && appt.venue.toLowerCase().includes(searchTerm.toLowerCase())) ||
      ...
      const matchesCategory = categoryFilter === 'All' ||
        (categoryFilter === 'Official' && isOfficial) ||
        (categoryFilter === 'Citizen' && !isOfficial)
      ...
      <section className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-200/80 dark:border-slate-800/80 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1 max-w-2xl">
            ...
            <div className="flex items-center bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200/30 dark:border-slate-800/30 shrink-0">
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => setCategoryFilter('Citizen')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1 ${
                  categoryFilter === 'Citizen'
                    ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/20'
                    : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                }`}
              >
                <span>👤</span>
                <span>Public Day</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <table className="w-full border-collapse text-left text-sm">
            <thead>
              <tr className="bg-slate-50/20 dark:bg-slate-950/20 border-b border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 uppercase tracking-wider font-bold">
                <th className="px-6 py-4">Session / Client</th>
                <th className="px-6 py-4">Agenda / Subject</th>
                    ...
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-white">
                      <div className="flex items-center gap-2">
                        <div>
                          ...
                          <div className="text-[10px] font-normal text-slate-400 dark:text-slate-500 mt-0.5">{appt.email ? `${appt.email} • ${appt.phone}` : appt.phone}</div>
                  ...
                  <div className="flex items-center gap-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isOfficial ? (
                          <span className="text-[8px] px-1.5 py-0.5 rounded-full font-bold bg-purple-100 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300">
                            🏢 Official
                          </span>
                        ) : (
                          <span className="text-[8px] px-1.5 py-0.5 rounded-full font-bold bg-blue-100 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300">
                            👤 Citizen
                          </span>
                        )}
                      </div>
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">{appt.reason}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{appt.email ? `${appt.email} • ${appt.phone}` : appt.phone}</p>
                      {!isOfficial && appt.nic && appt.nic !== 'OFFICIAL' && (
